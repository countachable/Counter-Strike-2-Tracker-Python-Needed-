...
MY_PLAYER_NAME = "The Goodest Goy" # Your player name for filtering GSI data
...
current_match_wins = 0 # NEW: Track wins for the current match
current_match_losses = 0 # NEW: Track losses for the current match
is_not_playing_message_logged = False # NEW: Flag to prevent repeated messages
...
current_match_wins_label = None # NEW: Label for wins
current_match_losses_label = None # NEW: Label for losses
current_match_team_label = None # NEW: Label for current team
...
def gsi_data_handler():
    ...
    global my_team_in_match
    global current_match_wins, current_match_losses
    global is_not_playing_message_logged
    ...
    # Check if player is active in a tracked game mode
    is_my_player_active_at_start = (
        my_player_name_from_gsi == MY_PLAYER_NAME and
        my_player_activity in ["playing", "textinput", "menu"] and # Added 'menu' to not fail on warmups
        game_mode in ["competitive", "scrimcomp", "premier", "casual"] and
        game_phase in ["live", "intermission", "warmup"] # Added 'warmup' phase
    )
    ...
    if is_in_competitive_match and game_phase == "gameover":
        ...
        current_match_wins = 0 # NEW: Reset wins
        current_match_losses = 0 # NEW: Reset losses
        is_not_playing_message_logged = False # NEW: Reset flag
    ...
    if not is_in_competitive_match and is_my_player_active_at_start:
        ...
        last_processed_round_num = -1
        current_match_xp_gain = 0
        current_match_wins = 0 # NEW: Reset wins for new match
        current_match_losses = 0 # NEW: Reset losses for new match
        ...
        log_to_console("Please select your team (CT or T) on the GUI.")
        player_record = get_player()
    ...
    # --- Check for early exit if not in a tracked game mode ---
    elif not is_in_competitive_match:
        if not is_not_playing_message_logged:
            ...
            is_not_playing_message_logged = True
        return jsonify({"status": "success", "message": "Not playing as specified player or not competitive/casual, skipped processing."})


    # --- Process Live Game Data for XP (This block ONLY executes if is_in_competitive_match is True) ---
    # The check for player name and activity is now handled by the start logic, so we can assume we're in the right game.
    ...
    if is_in_competitive_match and current_round_num != -1 and \
       ...
       (round_phase == 'over' or round_phase == 'freezetime' or game_phase == 'intermission'):
        ...
        # Check round winner and update wins/losses ONLY if a team has been selected
        if my_team_in_match:
            round_win_team = round_data.get('win_team', '').lower()
            if round_win_team == my_team_in_match:
                current_match_wins += 1
                log_to_console(f"  Round {current_round_num} was a WIN! Total wins: {current_match_wins}")
            elif round_win_team and round_win_team != my_team_in_match:
                current_match_losses += 1
                log_to_console(f"  Round {current_round_num} was a LOSS. Total losses: {current_match_losses}")
            else:
                log_to_console(f"  Round {current_round_num} result was a DRAW or WINNER UNDEFINED. Current wins: {current_match_wins}, losses: {current_match_losses}")
        else:
            log_to_console(f"  WARNING: Player team not set. Round win/loss not tracked.")
...
def finalize_and_save_match_data(map_data, final_my_stats, my_player_team_stored): # Renamed for clarity
    ...
    elif my_player_team_stored == '':
        win_status = None
        log_to_console("MATCH END: Player team not set. No win/loss XP applied.")
    else:
        win_status = False
        current_match_xp_gain -= XP_PENALTY_LOSS
        log_to_console(f"MATCH END: Applied LOSS penalty XP: -{XP_PENALTY_LOSS}. Reason: My stored team did not have higher score.")
...
def set_team(team):
    """Callback function to set the player's team for the current match."""
    global my_team_in_match
    my_team_in_match = team
    log_to_console(f"Manually set player team to: '{my_team_in_match}'")
...
def update_ui_stats():
    ...
    global current_match_wins_label, current_match_losses_label, current_match_team_label
    ...
    # Update current match team label
    current_match_team_label.config(text=f"Current Match Team: {my_team_in_match.upper() if my_team_in_match else '--'}")
    ...
    if is_in_competitive_match: # Only update if we are actively playing a match
        ...
        # NEW: Update the wins/losses labels
        current_match_wins_label.config(text=f"Match Wins: {current_match_wins}")
        current_match_losses_label.config(text=f"Match Losses: {current_match_losses}")
        ...
        # NEW: Reset the wins/losses labels
        current_match_wins_label.config(text="Match Wins: --")
        current_match_losses_label.config(text="Match Losses: --")
...
def setup_ui():
    ...
    # Labels for current match wins/losses
    current_match_wins_label = ttk.Label(stats_frame, text="Match Wins: --", font=("Segoe UI", 12))
    current_match_wins_label.grid(row=9, column=0, sticky=tk.W, pady=2)

    current_match_losses_label = ttk.Label(stats_frame, text="Match Losses: --", font=("Segoe UI", 12))
    current_match_losses_label.grid(row=10, column=0, sticky=tk.W, pady=2)

    # New Team Selection Frame
    team_frame = ttk.LabelFrame(main_frame, text="Manual Team Selection", padding="10")
    team_frame.grid(row=1, column=1, padx=5, pady=5, sticky=(tk.W, tk.E, tk.N, tk.S))
    
    ct_button = ttk.Button(team_frame, text="Counter-Terrorists", command=lambda: set_team('ct'))
    ct_button.grid(row=0, column=0, padx=5, pady=5)
    
    t_button = ttk.Button(team_frame, text="Terrorists", command=lambda: set_team('t'))
    t_button.grid(row=1, column=0, padx=5, pady=5)
    
    current_match_team_label = ttk.Label(team_frame, text="Current Match Team: --", font=("Segoe UI", 12, "italic"))
    current_match_team_label.grid(row=2, column=0, padx=5, pady=5)
